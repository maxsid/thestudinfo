        ...
        function checkSupport(){
            ...
            switch ($browser) {
                case Browser::BROWSER_IE:
                    if (!$this->checkVersion($version,"10",2))
                        return false;
            }
            return true;
        }

        function checkVersion($userVersion,$minRequiredVersion, $levelCheck = null) {
            if (empty($userVersion) | empty($minRequiredVersion)){
                return null;
            }
            $userVersionArray = array();
            $requiredVersion = array();
            $regex = '/[\d]+/';
            preg_match_all($regex,$userVersion,$userVersionArray);
            $userVersionArray = $userVersionArray[0];
            preg_match_all($regex,$minRequiredVersion,$requiredVersion);
            $requiredVersion = $requiredVersion[0];
            if (is_null($levelCheck)) {
                $levelCheck = count($userVersionArray) > count($requiredVersion)
                    ? count($userVersionArray) : count($requiredVersion);
            }
            for ($i = 0; $i < $levelCheck;$i++){
                if (count($userVersionArray) - 1 < $i) {
                    $userVersionArray[] = "0";
                }
                if (count($requiredVersion) - 1 < $i) {
                    $requiredVersion[] = "0";
                }
                if ($userVersionArray[$i] < $requiredVersion[$i]){
                    return false;
                }
            }
            return true;
